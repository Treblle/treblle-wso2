...
### How It Works
1. `handleRequest()` reads the `treblle_mask_keywords` property from the MessageContext (tries direct property, `additionalProperties` map, then `api.ut.additionalProperties` map)
2. Parsed keywords are cached by API UUID in a `ConcurrentHashMap` for performance
